...
$app->get('/get-report', function (Request $request, Response $response) { {
        ...
        try {
            ...
            if ($selectedOption == 'SalesByPharmacy') {
                $sqlInvoices = "SELECT CUSTDES, sum(QTY) as QTY,ROUND(sum(NET_VALUE), 2) as Gross_value , sum(LINE_DISC) as Discount, sum(OVERALL_DISC) as over_all_discount, ROUND(sum(TOTAL_VALUE_INCL_VAT + LINE_DISC + OVERALL_DISC - NET_VALUE), 2) as VAT, sum(TOTAL_VALUE_INCL_VAT) as TOTAL_VALUE_INCL_VAT  FROM bi.fact_invoiceitems inner join dim_customers on dim_customers.CUST = fact_invoiceitems.CUST where fact_invoiceitems.IVDATE between $formattedStartDate AND $formattedEndDate and VENDOR_ID=$supNum group by  dim_customers.CUSTDES";
            } else if ($selectedOption == 'SalesByBrand') {
                $sqlInvoices = "SELECT parts.DEXT_BRAND as BRAND, sum(QTY) as QTY,ROUND(sum(NET_VALUE), 2) as Gross_value , sum(LINE_DISC) as Discount, sum(OVERALL_DISC) as over_all_discount, ROUND(sum(TOTAL_VALUE_INCL_VAT + LINE_DISC + OVERALL_DISC - NET_VALUE), 2) as VAT, sum(TOTAL_VALUE_INCL_VAT) as TOTAL_VALUE_INCL_VAT  FROM bi.fact_invoiceitems inner join dim_part ON dim_part.PART = fact_invoiceitems.PART inner join parts on parts.PARTNAME = dim_part.PARTNAME where fact_invoiceitems.IVDATE between $formattedStartDate AND $formattedEndDate and VENDOR_ID=$supNum group by  parts.DEXT_BRAND";
            } else if ($selectedOption == 'SalesByTown') {
                $sqlInvoices = "SELECT dim_customers.STATEA as CITY, sum(QTY) as QTY,ROUND(sum(NET_VALUE), 2) as Gross_value , sum(LINE_DISC) as Discount, sum(OVERALL_DISC) as over_all_discount, ROUND(sum(TOTAL_VALUE_INCL_VAT + LINE_DISC + OVERALL_DISC - NET_VALUE), 2) as VAT, sum(TOTAL_VALUE_INCL_VAT) as TOTAL_VALUE_INCL_VAT  FROM bi.fact_invoiceitems inner join dim_customers ON dim_customers.CUST = fact_invoiceitems.CUST where fact_invoiceitems.IVDATE between $formattedStartDate AND $formattedEndDate and VENDOR_ID=$supNum group by  dim_customers.STATEA";
            } else if ($selectedOption == 'SalesByProduct') {
                $sqlInvoices = "SELECT dim_part.PARTDES as PARTNAME, sum(QTY) as QTY,ROUND(sum(NET_VALUE), 2) as Gross_value , sum(LINE_DISC) as Discount, sum(OVERALL_DISC) as over_all_discount, ROUND(sum(TOTAL_VALUE_INCL_VAT + LINE_DISC + OVERALL_DISC - NET_VALUE), 2) as VAT, sum(TOTAL_VALUE_INCL_VAT) as TOTAL_VALUE_INCL_VAT  FROM bi.fact_invoiceitems inner join dim_part ON dim_part.PART = fact_invoiceitems.PART where fact_invoiceitems.IVDATE between $formattedStartDate AND $formattedEndDate and VENDOR_ID=$supNum group by  dim_part.PARTDES";
            } else if ($selectedOption == 'SalesByCPPB') {
                $sqlInvoices = "SELECT CUSTDES, dim_customers.STATEA as CITY, parts.DEXT_BRAND as BRAND, dim_part.PARTDES, sum(QTY) as QTY,ROUND(sum(NET_VALUE), 2) as Gross_value , sum(LINE_DISC) as Discount, sum(OVERALL_DISC) as over_all_discount, ROUND(sum(TOTAL_VALUE_INCL_VAT + LINE_DISC + OVERALL_DISC - NET_VALUE), 2) as VAT, sum(TOTAL_VALUE_INCL_VAT) as TOTAL_VALUE_INCL_VAT  FROM bi.fact_invoiceitems inner join dim_customers on dim_customers.CUST = fact_invoiceitems.CUST inner join dim_part ON dim_part.PART = fact_invoiceitems.PART
                ...
                //nicosia
                // $statea = 'ΛΕΥΚΩΣΙΑ';
                $sqlInvoicesNicosia = "SELECT CUSTDES, dim_customers.STATEA, sum(QTY) as QTY, ROUND(sum(NET_VALUE), 2) as Gross_value, sum(LINE_DISC) as Discount, sum(OVERALL_DISC) as over_all_discount, ROUND(sum(TOTAL_VALUE_INCL_VAT + LINE_DISC + OVERALL_DISC - NET_VALUE), 2) as VAT, sum(TOTAL_VALUE_INCL_VAT) as TOTAL_VALUE_INCL_VAT FROM bi.fact_invoiceitems INNER JOIN dim_customers ON dim_customers.CUST = fact_invoiceitems.CUST WHERE fact_invoiceitems.IVDATE BETWEEN :startDate AND :endDate AND VENDOR_ID = :supNum GROUP BY dim_customers.CUSTDES";
